...
const WhyHueneuSection = () => {
  ...
  useEffect(() => {
    ...
    // Store current refs to ensure cleanup function uses the same values
    const currentSectionRef = sectionRef.current;
    const currentHeaderRef = headerRef.current;
    const currentText1Ref = text1Ref.current;
    const currentText2Ref = text2Ref.current;
    const currentText3Ref = text3Ref.current;

    if (currentSectionRef) observers.push(createObserver(sectionRef, setIsVisible));
    if (currentHeaderRef) observers.push(createObserver(headerRef, setHeaderVisible));
    if (currentText1Ref) observers.push(createObserver(text1Ref, setText1Visible));
    if (currentText2Ref) observers.push(createObserver(text2Ref, setText2Visible));
    if (currentText3Ref) observers.push(createObserver(text3Ref, setText3Visible));

    // Cleanup observers on component unmount
    return () => {
      observers.forEach(obs => {
        // Check if target elements still exist before trying to unobserve or disconnect
        // This check is implicitly handled by disconnect, but explicit unobserve might need it if refs were passed directly.
        obs.disconnect();
      });
    };
  }, []);
